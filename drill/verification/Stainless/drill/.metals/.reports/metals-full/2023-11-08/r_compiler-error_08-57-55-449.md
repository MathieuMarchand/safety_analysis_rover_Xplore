...
object drill {
 ...
 (currState == Up() && currHeight != drillHeight)))
  ...
  def loop(s: DrillState, nb: BigInt): (DrillState, BigInt) = {
    val v = action(s, Launch())
    (v, nb+1)
  }.ensuring((s, n) => sget()@@.currState == Idle() && n <= 5)
  ...
  def action(s: DrillState, instruction:Instruction): Option[DrillState] = {
    ...
    instruction match
        ...
        case Abort() => Some(s.copy(currState = Retract(), aborted = true))
        case _ => None()
  }
